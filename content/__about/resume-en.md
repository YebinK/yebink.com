---
title: 'about'
date: 2020-10-04 03:38:00
lang: 'en'
---



<div class="title-border"><h1 style="font-size: 2.5rem; padding: 0px; border-bottom: none; margin-bottom: none; padding-top: 2px;">
  김예빈 (Yaebin Kim)
  </h1></div>




<h2 style="border-bottom: none; margin-bottom: none;">
  안녕하세요! 매일 성장하는 백엔드 개발자 <span style="color:#5BB8B5">김예빈</span>입니다 👋
  </h2>
<a href="https://github.com/yebink"><img src="http://ghchart.rshah.org/yebink" alt="YebinK's Github chart" /></a>



<br/>


|             |                                          |
| :---------: | ---------------------------------------- |
| **GitHub**  | https://github.com/yebink                |
|  **Blog**   | <https://ilovepotato.tistory.com>        |
| **Contact** | [yebin825@gmail.com](yebin825@gmail.com) |

<br/>



## 저는 이런 사람입니다 !

<span class="highlight-keyword"><mark>SELF-MOTIVATED 🤓</mark></span>

**저는 개발을 사랑합니다.** 

매일 느낄 수 있는 성취감은 *개발자만의 특권* 이라고 생각합니다! 끊임없이 고민하고 연구해 성과를 내는 모든 과정 속에서 성취감을 느끼고, 이는 제 삶의 원동력입니다.

<br/>

<span class="highlight-keyword"><mark>COLLABORATIVE 🎨</mark></span>

**저는 협업을 좋아합니다.** 

여러 사람들이 만나 협업을 통해 시너지가 발생할 때 큰 희열을 느낍니다. *혼자 하면 빨리 가고, 함께 하면 멀리 간다* 는 말이 있듯이, 협업을 통해 함께 성장하는 것을 매우 좋아합니다.

<br/>

<span class="highlight-keyword"><mark>SPEEDY 🚀</mark></span>

**저는 빠르게 배웁니다.**

저는 남들보다 조금 빠른 속도를 갖고 있습니다. <span style="color: #a0a0a0; text-decoration-thickness: 0.5px;">~~그래서인지 우아한테크코스 내에서 타자도 가장 빠릅니다.(ㅎㅎ)~~</span> 이러한 제 장점을 활용해 새로운 기술이나 개념도 빠르게 파악하고 체득하는 데에 힘을 쏟고 있습니다. 

<br/>

<span class="highlight-keyword"><mark>STEADY 🌞</mark></span>

**저는 어제도, 오늘도, 내일도 성장합니다.** 

고등학교 시절 가장 먼저 등교해 교실 불 켜기, 대학교 시절에는 전공 공부를 열심히 해서 과탑도 경험했으며, 최근에는 일일 커밋으로 꾸준히 성장하는 자세를 유지하고 있습니다.

<br/>

<span class="highlight-keyword"><mark>I LOVE POTATO 🥔</mark></span>

**저는 감자튀김을 사랑합니다.**

세상에 감자튀김만큼 맛있는 음식은 없다고 생각합니다. 주로 버<span style="color: #f0f0f0;">거</span>킹의 두꺼운 느낌보다는 맥<span style="color: #f0f0f0;">도</span>날<span style="color: #f0f0f0;">드</span>의 얇은 느낌을 선호하지만, 크게 가리지 않고 잘 먹습니다.



<br/>



## Projects

### [도란도란](https://dorandoran.io) <span class="date">(2020.06 ~ ing)</span>

**지도를 기반으로 궁금하거나 알리고 싶은 정보를 공유하는 서비스**

[Repository Link](https://github.com/woowacourse-teams/2020-doran-doran/issues)

**담당 업무**

- 아이디어 초기 기획
- 핵심 엔티티 설계 및 연관관계 설정
- 소중한 데이터 보호를 위한 엔티티의 Soft Delete 구현
- QueryDSL 도입 및 트러블슈팅
- Jenkins를 이용한 배포 자동화
- SonarQube를 이용한 정적 코드 분석
- 사용자 피드백을 반영해 검색창에서 위치 검색 가능하도록 변경
- Vue.js 학습 및 Vuex를 통한 상태관리 진행

<br/>

**기술 스택**

- BackEnd: Java 8, Spring Boot 2.3, Spring Data JPA, QueryDSL, Gradle, JUnit5
- FrontEnd: Vue.js 2.0, JS (ES6)
- Infra: Jenkins, NginX, MariaDB, AWS EC2

<br/><br/>

### 이만 머지할게요 <span class="date">(2020.05 ~ ing)</span>

**우아한테크코스 크루 간의 상호 코드리뷰 서비스**

**담당 업무**

- 아이디어 초기 기획 및 디자인

- 핵심 도메인 설계 및 구현
- 멀티 모듈로 분리
- 프론트 작업 중

**기술 스택**

- BackEnd: Java 8, Spring Boot 2.3, Spring Data JPA, Gradle, Multi Module, JUnit5
- FrontEnd: Vue.js 2.0, JS (ES6)
- Infra: Jenkins, NginX, MariaDB, EC2

<br/>



## Skills

### <mark>클린코드 역량</mark>

- **[우아한테크코스](https://github.com/YebinK/woowacourse-collection)**
  - TDD 사이클을 반복하며 유지보수가 용이한 코드 작성
  - 객체의 책임을 줄이고, 상태와 행위를 한 곳에서 관리하도록 만드는 것을 좋아함
  - 리뷰어, 페어, 팀원들과의 지속적인 코드리뷰를 통해 발전

<div style="margin: 20px;"></div>

### <mark>협업 역량</mark>

- **댄스 동아리 Blank**
  - 대학교 시절 댄스 동아리 활동을 통해 "협업"의 중요성 파악
  - 완성도 있는 안무를 위해 한 번도 빠지지 않고 연습에 참여, 성공적인 버스킹 활동으로 이어짐

<div style="margin: 20px;"></div>

- **성신여자대학교 해외탐방 프로그램 글로벌프론티어**
  - 7명의 팀원들과 밤낮으로 실리콘밸리 탐방 준비 및 100여 개의 스타트업과 컨택
  - 해외 스타트업으로부터 팀워크를 인정받아 MOU 체결 및 인턴십 진행
  - 팀원들과 함께 준비한 보고서로 1등 수상

<div style="margin: 20px;"></div>

- **[우아한테크코스](https://github.com/YebinK/woowacourse-collection)**
  - 지속적인 페어 프로그래밍과 팀 프로젝트를 통해 "개발자로서 협업하는 방법" 터득

<div style="margin: 20px;"></div>

### <mark>문제 해결 역량</mark>

- **[도란도란 프로젝트](https://dorandoran.io)**
  - 2개 이상의 컬렉션에 대해 Fetch Join을 사용하던 중 문제가 발생. 책과 공식문서 등을 활용해 batch size로 해결, 팀 [위키](https://github.com/woowacourse-teams/2020-doran-doran/wiki/%EC%97%AC%EB%9F%AC-%EA%B0%9C%EC%9D%98-%EC%BB%AC%EB%A0%89%EC%85%98%EC%9D%84-FetchJoin-%EC%8B%9C-%EC%84%B1%EB%8A%A5-%EC%B5%9C%EC%A0%81%ED%99%94)에 정리해 팀원들과 공유

<div style="margin: 20px;"></div>

### <mark>사용자 중심의 시스템 개선 역량</mark>

- **[도란도란 프로젝트](https://dorandoran.io)**
  - 사용자 피드백 취합, 사용자 패턴 분석을 통해 실 사용자들이 "원하는" 기능에 집중
  - 결국 서비스의 목적은 "고객창출"과 "고객만족"임을 인지하고, 지속적으로 개선 중

<div style="margin: 20px;"></div>

## Education
- **성신여자대학교 정보시스템학과 (부: 컴퓨터소프트웨어학과)** 졸업 예정 <span class="date">(~ 2021.06)</span>

   `전공 4.16 / 4.5`  전체 4.18 / 4.5

- **경기외국어고등학교 일본어과** 졸업 <span class="date">(2013.03 ~ 2016.02)</span>

<br/>

## Prize

- ⟨팀 프로젝트가 나에게 남긴 것⟩ 레벨 3 [글쓰기](https://woowabros.github.io/techcourse/2020/08/28/techcourse-level3-retrospective.html) 수상, **우아한테크코스** <span class="date">(2020.08)</span>
- ⟨외국인 관광객의 농촌 체험 어플, 팜스테이⟩ 농협 해커톤 장려상, **농협은행** <span class="date">(2019.09)</span>

- ⟨음성인식 애완견 헬퍼, NUGU PET⟩ NUGU Dev Days 장려상, **SK텔레콤** <span class="date">(2019.08)</span>
- ⟨카페 리뷰 어플, 커뷰⟩ SW챌린지 콘테스트 우수상, **KOSA** <span class="date">(2018.01)</span>

- ⟨지하철 중간점 찾기 프로그램⟩ 소프트웨어 경진대회 장려상, **성신여자대학교** <span class="date">(2017.08)</span>
- ⟨실리콘밸리 스타트업 탐방⟩ 글로벌프론티어 대상, **성신여자대학교** <span class="date">(2017.08)</span>

<br/>

## Certificate
- **TOEIC**  `910`
  - 영어로 된 공식문서도 거뜬합니다. 💪

<div style="margin: 20px;"></div>

- **ACA Premiere CS6**
  - 취미로 영상 편집도 합니다. 유튜브로 돈을 벌 수 있는 세상이니까요. 🎬



<br/><br/>



<div align="center" style="font-size:12px;">Copyright © 2020 by YebinK <br/> All Rights Reserved.</div>

